// src/app/api/admin/remove-claim/route.ts
...
async function logSecurityEvent(payload: {
  ...
  try {
    ...
    await adminDb.collection("securityLogs").add({
      type: "admin_remove_claim_attempt",
      ok: payload.ok,
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // ✅ REMOVE ADMIN
    await adminAuth.setCustomUserClaims(body.uid, { admin: false });

    await logSecurityEvent({
      ok: true,
      reason: "admin_claim_removed",
      uid: body.uid,
      ip,
      ua,
    });
